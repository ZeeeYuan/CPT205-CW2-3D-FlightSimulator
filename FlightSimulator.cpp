...
void Texture::ReadImage(const char path[256]) {
	...
	// Create ith texture
	glGenTextures(1, &id);
	// Bind coresponding texture to texture ID
	glBindTexture(GL_TEXTURE_2D, id);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, imageWidth, imageHeight, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, p[0]);
	// GL_MODULATE: combine the texture with material color; GL_REPEAT: Repeat wrapping; GL_LINEAR: Linear interpolation
...
void drawSkybox()
{
	...
	gluQuadricTexture(quadObject, true); // Generate texture co-ordinates
